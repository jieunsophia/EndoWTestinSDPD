...
etaset=[0 0 0]; [num_hypothesis_eta , ~]=size(etaset);
...
% Step 1. Data generating process for Y_exo, Z_exo, and fixed effects
for nT_set_i=1:length_nT
    n=nT_set(nT_set_i,1); T=nT_set(nT_set_i,2)+1; % Include time=0 in a part of dgp for the time being
    L=n*T; nt=L; 

    % Deterministic random variables: X1_exo, X2_exo, Y_exo, Z_exo, fixed effects
    rng(2022)
    Y_exo=normrnd(0,1,[n 1]); % Initial conditions
    Z_exo=mvnrnd(0*ones(p,1),corr_z,n); % Initial conditions

    % X1,X2
    x1=mvnrnd(zeros(kx1+2,1), [eye(kx1) corr_ife*ones(kx1,1) corr_tfe*ones(kx1,1); ...
                              corr_ife*ones(1,kx1) 1 0; ...
                              corr_tfe*ones(1,kx1) 0 1], L);
    X1_exo=x1(:,(1:kx1)); c10=x1(:,(kx1+1)); a10=x1(:,(kx1+2));
    ...
    X2_exo=x2(:,(1:kx2)); c20=x2(:,(kx2+1):(kx2+p)); a20=x2(:,(kx2+p+1):(kx2+p+p));
    ...
    twoway_fe_corr=[corr(X1_exo,c1)' corr(X1_exo,a1)' ...
                    reshape(corr(X2_exo,c2),[],1)' reshape(corr(X2_exo,a2),[],1)']; % nonzero 
    ...
    for w_type=1:2
        ...
        for i=1:n
            for j=i+1:n
                W_exo_e(i,j)=1/norm(Z_exo(i,:)-Z_exo(j,:));
            end
        end
        W_exo_e=W_exo_e+W_exo_e';    
        W_exo=normw(W_d.*W_exo_e);            
        ...
        for delta_i=1:nrow_deltaset % Loop for delta
            ...
            for eta_i=1:num_hypothesis_eta % Loop for eta
                ...
                for sim_i=1:sim % Loop for simulation
                    ...
                    for t=1:T
                        if t==1
                            Z(1:n,:)=Z_exo*kappa0+X2_exo(1:n,:)*Gamma0+c2(1:n,:)+a2(1:n,:)+e(1:n,:); % Z_0
                        else
                            Z((t-1)*n+1:t*n,:)=Z((t-2)*n+1:(t-1)*n,:)*kappa0...
                                                +X2_exo((t-1)*n+1:t*n,:)*Gamma0...
                    ...
                    % Generate Y
                    ell=zeros(L,1); ell(1:n)=gamma0*Y_exo+rho0*W_exo*Y_exo;

                    JT=speye(T)-ones(T,1)*ones(T,1)'/T; Jn=speye(n)-ones(n,1)*ones(n,1)'/n;
                    JL=kron(JT,Jn);
                    SL=speye(L)-W;

                    Y=SL\(X1_exo*beta0+ell+c1+a1+v);
                    ...
                    % Observable data
                    Z=Z((n+1):L,:); % Z_0 unobserved
                    Y=Y((n+1):L,:); % Y_0 unobserved
                    W1=W1((n+1):L, (n+1):L); W2=W2((n+1):L, (n+1):L); W3=W3((n+1):L, (n+1):L); 
                    w=w(:,((2*n+1):(n*(T+1)))); % w_exo and w_0 unobserved   
                    X1=X1_exo((n+1):L,:); % x_10 unobserved
                    X2=X2_exo((n+1):L,:); % x_20 unbserved
                    T=T-1; % Exclude time=0
                    ...
                    Y_stack=[Y0_proxy; Y];
                    Z_stack=[Z0_proxy; Z];     
                    w_stack=[w1,w]; % Replicate w1 as w0

                    % Data for use             
                    y=Y_stack; x=X1; z=Z_stack; zx=X2; W=w_stack;           
                    ...
                    Syt=yt-lambda*ysl;
                    epsilon=zt-R2t*Phi2;
                    R1e=R1t(:,(3:(2+kx1))); % Under the null: gamma0=rho0=0, delta0=0
                    phi1_beta = R1e\Syt;
                    phi1 = [0 0 phi1_beta']'; % gamma0=rho0=0, restricted under the null
                    xi=Syt-R1t*phi1;
                    xipxi = xi'*xi;
                    sigma=xipxi/L;
                    ...
                    % Back to original setup for the next iteration
                    T=T+1; 
                    L=n*T; nt=L; 
                    end % Simulation ends

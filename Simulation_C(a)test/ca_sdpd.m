...
for nT_set_i=1:length_nT
    ...
    X1=x1(:,(1:kx1)); c10=x1(:,(kx1+1)); a10=x1(:,(kx1+2)); [~,k1]=size(X1);
        ...
        for delta_i=1:nrow_deltaset % Loop for delta
            ...
            for eta_i=1:num_hypothesis_eta % Loop for eta
                ...
                for sim_i=1:sim % Loop for simulation
                    ...
                    [pout,like,exitflag,output]=fminsearch('f_sdpd_endo_Wt_ca',parm,options,yt,ysl,R1t,zt,R2t,W); % restricted likelihood function under the null: lambda0=gamma0=rho0=0
                    ...
                    lambda=lambda1;
                    phi1=phi11; gamma=phi1(1); rho=phi1(2);
                    delta=delta1;
                    phi2=phi21;
                    sigma=sigma1;
                    ...
                    I_deltaphi1=epsilon'*R1t;
                    I_deltasigma=zeros(p,1);
                    
                    I_phi1phi1=R1t'*R1t; % phi1
                    I_phi1sigma=zeros(k1+2,1);
                    
                    I_sigmasigma=(1/sigma)*(0.5*L-t-n+1); % sigma
        
                    I_deltadelta=epsilon'*epsilon/(L*sigma); % delta
                    I_deltapsi=[I_deltalambda I_deltaphi1 I_deltasigma]/(L*sigma); %
                    I_psipsi=[I_lambdalambda I_phi1lambda' I_sigmalambda'; ...
                              I_phi1lambda I_phi1phi1 I_phi1sigma; ...                                                 ...
                              I_sigmalambda I_phi1sigma' I_sigmasigma]/(L*sigma); %

                    % Bias terms
                    D_1delta=zeros(p,1); %
                    D_2delta=zeros(p,1); %

                    D_1lambda=-1/(n-1)*trace(G1*kron(ones(T,1)*ones(T,1)'/T,Jn));
                    D_1gamma=trace(G2*kron(ones(T,1)*ones(T,1)'/T,Jn));
                    D_1rho=trace(G3*kron(ones(T,1)*ones(T,1)'/T,Jn));
                    D_1beta=zeros(k1,1);
                    D_1phi1=1/(n-1)*[D_1gamma,D_1rho,D_1beta']';
                    D_1sigma=-1/(2*sigma);
                    D_1psi=sqrt(n/T)*[D_1lambda; D_1phi1; D_1sigma]; %
                        
                    D_2lambda=-1/T*trace(G1*kron(speye(T),ones(n,1)*ones(n,1)'/n));
                    D_2gamma=0;
                    D_2rho=0;
                    D_2beta=zeros(k1,1);
                    D_2phi1=[D_2gamma, D_2rho, D_2beta']';
                    D_2sigma=-1/(2*sigma);
                    D_2psi=sqrt(T/n)*[D_2lambda; D_2phi1; D_2sigma]; %
                    ...
                    L_phi1=RJXi;
                    L_sigma=-L/2+1/(2*sigma)*xi'*xi;
                    L_psi=[L_lambda; L_phi1; L_sigma]/(L*sigma); %

                    % Test statistic
                    C_delta=L_delta-(D_1delta+D_2delta)/sqrt(L)+I_deltapsi*(I_psipsi\(D_1psi+D_2psi))/sqrt(L); % 
                    I_dp=I_deltadelta-I_deltapsi*(I_psipsi\I_deltapsi'); % I_delta.psi
                    ca=L*C_delta'*(I_dp\C_delta);

                    % Record
                    RejectionRate_ca=[RejectionRate_ca (ca>critic1)];
                    end % Simulation ends

                    tElapsed_ca=[tElapsed_ca toc(tStart)];

                    % Estimated Rejection Rate (Size or Power) & time elapsed
                    RejectionRateHat_ca=sum(RejectionRate_ca)/sim;
                    
                    avg_tElapsed_RobustRS=mean(tElapsed_ca);

                    % Save
                    save(strcat(['ca_rej=',num2str(RejectionRateHat_ca),', avg_elapsed=',num2str(avg_tElapsed_RobustRS),', #sim=',num2str(sim),', n=',num2str(n),', T=',num2str(T),', p=',num2str(p),', delta0=',num2str(delta0'),', lambda0=',num2str(lambda0),', gamma0=',num2str(gamma0),', rho0=',num2str(rho0),', W_type=',W_type,', rng_type=',num2str(rng_type),', t_df=',num2str(t_df),'.mat']));
             end % eta
         end % delta
    end % w_type 
end